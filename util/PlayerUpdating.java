...
public class PlayerUpdating {
   ...
   */
	...
	{
		System.out.println("Updating player: " + p.details().USERNAME_AS_STRING);
		...
		for (int i = 0; i < World.curIndex; i++) {
			...
			{
				System.out.println("[1] Checking my List Player: " + ((p2 != null) ? (p2.details().USERNAME_AS_STRING) : ("NULL")));
		...
		{
			Player p2 = World.getPlayerByCheckingIndex(list[i]);
			System.out.println("[2] Checking my List Player: " + ((p2 != null) ? (p2.details().USERNAME_AS_STRING) : ("NULL")));
			if (p2 == null)
				continue;
  ...
  */
	...
	{
		System.out.println("Updating My Players Position " + p.details().USERNAME_AS_STRING);
